...
func TestLocalStorage_Save(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	storage, err := storage.NewLocalStorage(tempDir, "/files/")
	require.NoError(t, err)

	t.Run("save simple file", func(t *testing.T) {
		t.Parallel()
		content := []byte("hello world")
		fh := createFileHeader("test-storage.txt", content)
		path := "test.txt"
		...
		assert.Equal(t, "test-storage.txt", file.Filename)
...
func TestLocalStorage_Delete(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	s, err := storage.NewLocalStorage(tempDir, "/files/")
	require.NoError(t, err)
	...
	t.Run("delete existing file", func(t *testing.T) {
		...
		err = s.Delete(context.Background(), testFile)
	...
	t.Run("try to delete directory", func(t *testing.T) {
		...
		err = s.Delete(context.Background(), testDir)
		assert.Error(t, err)
		assert.True(t, errors.Is(err, storage.ErrIsDirectory))
	})
...
func TestLocalStorage_DeleteDir(t *testing.T) {
	...
	t.Run("delete directory with contents", func(t *testing.T) {
		...
		err = s.DeleteDir(context.Background(), testDir)
		assert.NoError(t, err)
	...
	t.Run("delete non-existent directory", func(t *testing.T) {
		...
		err := s.DeleteDir(context.Background(), path)
		assert.Error(t, err)
	...
	t.Run("try to delete file", func(t *testing.T) {
		...
		err = s.DeleteDir(context.Background(), singleFile)
		assert.Error(t, err)
		assert.True(t, errors.Is(err, storage.ErrNotDirectory))
	})
...
func TestLocalStorage_List(t *testing.T) {
	...
	t.Run("list directory contents", func(t *testing.T) {
		...
		entries, err := s.List(context.Background(), testDir)
		require.NoError(t, err)
	...
	t.Run("list file as directory", func(t *testing.T) {
		...
		entries, err := s.List(context.Background(), testFile)
		assert.Error(t, err)
		assert.True(t, errors.Is(err, storage.ErrNotDirectory))
		assert.Len(t, entries, 0)
	})
...
func TestLocalStorage_URL(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name:    "path with dots",
			baseURL: "/files/",
			path:    "./uploads/../uploads/storage.txt",
			want:    "/files/uploads/storage.txt",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			storage, err := storage.NewLocalStorage(t.TempDir(), tt.baseURL)
			require.NoError(t, err)
...
func TestLocalStorage_WithTimeout(t *testing.T) {
	...
	t.Run("upload times out", func(t *testing.T) {
		...
		storage, err := storage.NewLocalStorage(tempDir, "/files/", storage.WithLocalUploadTimeout(1*time.Nanosecond))
		require.NoError(t, err)
	...
	t.Run("upload completes within timeout", func(t *testing.T) {
		...
		storage, err := storage.NewLocalStorage(tempDir, "/files/", storage.WithLocalUploadTimeout(5*time.Second))
		require.NoError(t, err)

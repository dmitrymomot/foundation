...
type Processor struct {
	...
	strictMode   bool
	running      atomic.Bool // Prevents handler registration after Run() starts
	mu           sync.RWMutex
...
// IMPORTANT: All handlers must be registered BEFORE calling Run().
// Attempting to register handlers after Run() starts will panic.
...
func (p *Processor) Run(ctx context.Context) error {
	...
	p.logger.InfoContext(ctx, "processor shutting down, draining events")

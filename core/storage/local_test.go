...
func TestLocalStorage_Save(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	storage, err := storage.New(tempDir, "/files/")
...
func TestLocalStorage_Delete(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	s, err := storage.New(tempDir, "/files/")
	require.NoError(t, err)
...
func TestLocalStorage_URL(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			storage, err := storage.New(t.TempDir(), tt.baseURL)
			require.NoError(t, err)
...
func TestLocalStorage_WithTimeout(t *testing.T) {
	...
	t.Run("upload times out", func(t *testing.T) {
		...
		storage, err := storage.New(tempDir, "/files/", storage.WithLocalUploadTimeout(1*time.Nanosecond))
		require.NoError(t, err)
	...
	t.Run("upload completes within timeout", func(t *testing.T) {
		...
		storage, err := storage.New(tempDir, "/files/", storage.WithLocalUploadTimeout(5*time.Second))
		require.NoError(t, err)

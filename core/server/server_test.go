...
func TestServerDoubleRun(t *testing.T) {
	...
	port := getFreePort(t)
	server := server.New(fmt.Sprintf(":%d", port))
...
func TestServerGracefulShutdownTimeout(t *testing.T) {
	...
	port := getFreePort(t)
	server := server.New(fmt.Sprintf(":%d", port), server.WithShutdownTimeout(10*time.Millisecond))
